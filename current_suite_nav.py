...
cell = pdl.RGC_Neuron_nav('fohlmeister_geo_params_ultraultralight-nona.csv',ex_flag = True,ex_rand = 24)
...
if TEST_TYPE == 15:
	...
	plot_3d(outsoma/1000, dpplist, durlist, 'Threshold vs. DPP (Balanced) Amplitude and Duration (Soma)', 'DPP Amplitude [nA]', 'Pulse Duration [ms]')
	plot_3d(outaxon/1000, dpplist, durlist, 'Threshold vs. DPP (Balanced) Amplitude and Duration (Axon)', 'DPP Amplitude [nA]', 'Pulse Duration [ms]')
	plot_3d(abs(outaxon/outsoma), dpplist, durlist, 'Ratio of Threshold vs. DPP (Axon vs. Soma)', 'DPP Amplitude [nA]', 'Pulse Duration [ms]', 'Stimulation Threshold Ratio', myvmax = 6)

	save_obj(outsoma,'outsoma-minaxon-tt15-nav.p')
	save_obj(outaxon,'outaxon-minaxon-tt15-nav.p')

#make 3d dppmodbal plot (duration vs. dpp amp)
if TEST_TYPE == 151:
	cntlim = 30
	#durlist = [.2, .3, .6, 1, 2, 3]
	#durlist = [np.round(i,1) for i in np.linspace(.2,4,25)]
	durlist = [np.round(i,1) for i in np.linspace(.3,4,20)]
	dpplist = np.linspace(.01e3,3e3,20)
	outsoma = np.zeros((len(durlist),len(dpplist)))
	outaxon = np.zeros((len(durlist),len(dpplist)))
	for durind, dur in enumerate(durlist):
		dur1 = np.round(dur - .1,1)
		dur2 = .1
		stim_params = [mydelay, dur1, dur2, mysimtime, mydt, myrho]
		for dppind, dpp in enumerate(dpplist):
			print dur, dpp
			thr = ptl.get_thresh(cell, 'dppmodbal', init_amp, init_step, x_soma, y_soma_tp_rand_norm, stim_params, cnt_lim = cntlim, dpp_amp = dpp)
			print 'soma',thr
			outsoma[durind, dppind] = thr
			thr = ptl.get_thresh(cell, 'dppmodbal', init_amp, init_step, x_axon, y_axon_tp_rand_norm, stim_params, cnt_lim = cntlim,dpp_amp = dpp)
			print 'axon',thr
			outaxon[durind, dppind] = thr
	...
	save_obj(outsoma,'outsoma-minaxon-tt151-nav.p')
	save_obj(outaxon,'outaxon-minaxon-tt151-nav.p')
