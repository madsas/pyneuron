...
#		for sec in neuron.h.allsec():
		for i in self.nams: 
			self.parts[i].insert("spike_rat")
			h('ena=60.60')
			h('ek=-101.34')
			for seg in self.parts[i]: 
				[seg.spike_rat.gkbar,seg.spike_rat.gabar,seg.spike_rat.gcabar,seg.spike_rat.gkcbar,seg.spike_rat.gnabar]=list(self.params[i][3:-1])
		for sec in neuron.h.allsec():
			sec.insert("cad")
			h('ena=60.60')
			h('ek=-101.34')
...
#Calculate voltage at each compartment
	...
	def show(self,titl="Voltage vs. Time",showAx = 1, showStim=False):
		if self.go_already:
			...
			if showAx != 2:
				plt.plot(x, y)
			if showAx == 1 or showAx == 2: plt.plot(x, z)
			plt.title(titl)
			plt.xlabel("Time [ms]")
...
def make_dppbal(delay,dur1,dur2,dpAmp,stimAmp,total,dt,cathodic = 1):
	t = np.arange(total/dt)*dt
	unit1 = np.ones(np.round(dur1/dt))
	unit2 = np.ones(dur2/dt)
	if cathodic:
...
def calc_v(rho,i,x,y,x0,y0):
	...
	return neuron.h.Vector(np.array(i) * rho / (4 * np.pi * dist ) * 100) #in mV
