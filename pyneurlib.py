...
#Calculate voltage at each compartment
		...
		for i in self.cell.nams:
			for seg in self.cell.parts[i]:
				v_calculated = calc_v(rho,self.i_ext,x_dist,y_dist,x_seg,y_seg,self.cell.seg_len)
	...
	def show(self,titl="Voltage vs. Time",showAx = True, showStim=False):
		if self.go_already:
			x = np.array(self.rec_t)
			y = np.array(self.rec_v)
			z = np.array(self.rec_vax)
			plt.plot(x, y)
			if showAx: plt.plot(x, z)
			plt.title(titl)
			plt.xlabel("Time [ms]")
...
def make_dppbal(delay,dur1,dur2,dpAmp,stimAmp,total,dt):
	...
	unit1 = np.ones(np.round(dur1/dt))
	unit2 = np.ones(dur2/dt)
	dpp = np.concatenate((unit1*dpAmp, unit2*stimAmp, -unit2*(dpAmp + stimAmp)))
...
def calc_v(rho,i,x,y,x0,y0,len):
