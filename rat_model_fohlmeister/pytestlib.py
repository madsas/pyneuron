...
def get_max(cell, typmax, amp, x,y,stim_params, dppamp = 0): 
	if dppamp == 0: 
	    if typmax == 'dppbal': 
		[delay, dur1, dur2, dur3, simtime, dt, myrho] = stim_params
		[t,i]=fdict[typmax](delay,dur1,dur2,dur3,dppamp,amp,simtime,dt)
	    else:
...
def get_thresh_dppperc(cell, typ, init_amp, init_step, x, y, stim_params, dpp_perc = .8, spike_thr = 0, amp_step_thr = .025e3, cnt_lim = 15, cathodic = 1):
    #gets threshold and locks dpp:stimamp ratio
	if cathodic: #default is that you want negative threshold
		amp = init_amp if type(init_amp) != list else init_amp[1]
		step = init_step
		#downflg = 0
		...
		cnt = 0
		while step >= amp_step_thr: 
			[msom_rec, max_rec] = get_max(cell, typ, amp, x,y, stim_params,dppamp = amp*dpp_perc)
			if (max(msom_rec) > spike_thr or max(max_rec) > spike_thr): 
				#amp -= step
				amp += step
				#downflg = 1
